...
function createParamsPanel(env) {
	const panel = el("div", "position:fixed;left:8px;bottom:8px;width:268px;z-index:10000;box-sizing:border-box;background:var(--dsw-specific-sidebar-fill);border:1px solid var(--dsw-alias-border-l2);border-radius:10px;padding:10px 12px 12px;color:var(--dsw-alias-label-primary);font-size:12px;line-height:1.5;box-shadow:0 10px 28px rgba(20,30,45,.35);display:none;max-height:calc(100vh - 24px);overflow-y:auto;");
	document.body.appendChild(panel);
	const header = el("div", "display:flex;align-items:center;gap:8px;margin-bottom:8px;");
	header.appendChild(el("span", "flex:1;font-weight:600;font-size:12px;letter-spacing:.03em;color:var(--dsw-alias-label-primary)", "浮游生物参数"));
	const closeBtn = el("button", "border:0;background:none;cursor:pointer;color:var(--dsw-alias-label-secondary);font-size:14px;padding:2px 4px;", "✕");
	closeBtn.addEventListener("click", hide);
	header.appendChild(closeBtn);
	panel.appendChild(header);
	const toggleRow = el("div", "display:flex;align-items:center;gap:8px;margin-bottom:10px;padding-bottom:8px;border-bottom:1px solid var(--dsw-alias-border-l1);");
	const toggle = el("input", "accent-color:var(--dsw-alias-brand-primary);cursor:pointer;");
	...
	for (const spec of SPECS) {
		if (isDivider(spec)) {
			panel.appendChild(el("div", "margin:6px 0 7px;padding-top:7px;border-top:1px solid var(--dsw-alias-border-l1);color:var(--dsw-alias-label-tertiary);font-size:11px;letter-spacing:.05em;", spec.divider));
			continue;
		}
		const row = el("div", "display:flex;align-items:center;gap:8px;margin-bottom:7px;");
		row.appendChild(el("label", "flex:none;width:56px;color:var(--dsw-alias-label-secondary);font-size:11px;", spec.label));
		const input = el("input", "flex:1;min-width:0;accent-color:var(--dsw-alias-brand-primary);cursor:pointer;height:14px;");
		input.type = "range";
		input.min = String(spec.min);
		input.max = String(spec.max);
		input.step = String(spec.step);
		const value = el("span", "flex:none;width:58px;text-align:right;color:var(--dsw-alias-label-secondary);font-size:11px;font-variant-numeric:tabular-nums;");
	...
	const resetBtn = el("button", "width:100%;margin-top:4px;padding:5px 0;border:1px solid var(--dsw-alias-brand-primary);border-radius:6px;background:transparent;color:var(--dsw-alias-brand-primary);font-size:11px;cursor:pointer;", "恢复默认");
...
function rearrangeSidebar() {
	...
	for (const btn of buttons) if (btn !== keep) btn.remove();
